...
namespace UI {
	...
	{
		// отрисовка плавного перемещения машинки с одной точки на другую
	}

	void CDrawing::MoveCarsToStart( const std::vector<int>& numbers, const std::vector<CCoordinates>& newCoordinates )
	{
		// отрисовка исчезновения машинок
		// перенос машинок на старт
		// отрисовка появления машинок на старте
	}

	void CDrawing::DeleteCars( const std::vector<int>& numbers )
	{
		// отрисовка исчезновения машинок с номерами из numbers
	}
