...
int DynamicProgrammingStrategyFunc(const Map &map, const PlayerState &currentPlayer) {
	//map.print();
	CDynamicProgrammingStrategy strategy(map, currentPlayer);
	
	if (!strategy.HasNextState()) {
		return (int)GetMovementDirection(
			strategy.GetLastXDeviation(),
			strategy.GetLastYDeviation()
		);
	}

	auto step = strategy.GetNextPosition();
	return (int)GetMovementDirection(
		step.first - currentPlayer.GetX() - currentPlayer.GetXVelocity(), 
		step.second - currentPlayer.GetY() - currentPlayer.GetYVelocity()
	);
}
...
int AStarStrategyFunc(const Map &map, const PlayerState &currentPlayer) {
	...
	return (int)GetMovementDirection(
		step.position.first - currentPlayer.GetX() - currentPlayer.GetXVelocity(),
		step.position.second - currentPlayer.GetY() - currentPlayer.GetYVelocity()
	);
}
